...
As capturas usadas no README estão copiadas para `coder/docs/interface/` (nomes em minúsculas, sem acentos) e reduzidas a 1920 px de largura, sem recorte — todas a janela inteira. O `navegador no preview` foi recortado a uma versão anterior e voltou a ser a janela inteira; o texto do cartão do modo inspecionar fica pequeno nessa largura, e é por isso que ele também existe aqui no original, para quem precisar de o ler. Um recorte apertado chega a cortar listas a meio, e a leitura passa a ser a de um fragmento em vez da de um ecrã. Os originais ficam aqui, com o nome de origem. O `gimp.jpg` do README não vem daqui: é um fotograma do vídeo `criando desenho gimp.mp4`, porque a cena montada por script só aparece em movimento.
