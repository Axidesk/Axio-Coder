...
function resetEstadoDoTurno() {
    ...
    function addMessage(role, text) {
        ...
        if (role === 'ai') {
            ...
            prefix = `<div class="flex items-center gap-2 mb-1"><span class="${agentColor} font-bold cursor-pointer hover:underline" onclick="event.stopPropagation(); const logId = this.closest('.message').dataset.logId; if(logId) { const log = window.sessionLogsData.find(l => l.id === logId); if(log) { const pLog = document.getElementById('panel-log-session'); if(pLog.classList.contains('panel-col-closed') || pLog.classList.contains('log-recolhido')) document.getElementById('btn-open-log').click(); setTimeout(() => log.domElement.click(), 100); } }" title="Ver análise desta resposta">${agentName}</span><button class="btn-copy-msg text-[var(--text-mutado)] hover:text-[var(--text-branco)] transition-colors" title="Copiar mensagem"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg></button></div>`;
        }
