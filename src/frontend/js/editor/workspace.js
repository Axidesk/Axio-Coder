...
export function animateDock(dir) {
    ...
    alvo.addEventListener('animationend', aoTerminarAnimacaoDoDock);
    timerDaAnimacaoDoDock = setTimeout(limparAnimacaoDoDock, ANIMACAO_DO_DOCK_MS);
}

function aoTerminarAnimacaoDoDock(e) {
    if (e.target !== state.explorerView) return;
    limparAnimacaoDoDock();
}
...
function limparAnimacaoDoDock() {
    ...
    const alvo = state.explorerView;
    if (!alvo) return;
    alvo.classList.remove('dock-anim-right', 'dock-anim-bottom');
    alvo.removeEventListener('animationend', aoTerminarAnimacaoDoDock);
}
