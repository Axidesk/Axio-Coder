...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderQuestions() {
        let questionsHtml = '<div class="p-5 flex flex-col gap-6">';
...
// NOVA LÓGICA DE CÓPIA DAS FERRAMENTAS
    ...
    function createChildBalloon(title, htmlContent, snippetHtml, rawTextOld, rawTextNew, fileName, sessionTools) {
        ...
        header.addEventListener('click', (e) => {
            ...
            // Mantém a janela de raciocínio (Chain of Thought) aberta ao navegar
            // entre os itens da pilha de edições de uma rodada: apenas move a seleção.
            if (isShowingThoughts) {
                atualizarBotoesUndoRedo();
                return;
            }
    ...
    function renderHistoryCards() {
        ...
        dias.slice(0, 3).forEach(dia => {
            ...
            header.addEventListener('click', (e) => {
                ...
                // Se a janela de arquivos estiver aberta, atualiza-a com os arquivos
                // agregados do dia selecionado (navegação visual).
                if (!panelCol2.classList.contains('hidden')) {
                    openSessionAggregatedFiles(sessoes);
                }
            });
    ...
    // Agrega os arquivos de todas as rodadas de um dia do histórico e, se a janela
    // de arquivos já estiver aberta, atualiza-a com esses arquivos.
    async function openSessionAggregatedFiles(sessoes) {
        try {
            ...
            for (const sessao of sessoes) {
                let logs = sessionDetailCache[sessao.filename];
                if (logs === undefined) {
                    const resp = await fetch(`http://127.0.0.1:5000/api/session_detail?file=${encodeURIComponent(sessao.filename)}`);
                    const data = await resp.json();
                    if (data.error) continue;
                ...
                const groups = logs.map(saved => rebuildGroupFromSaved(saved));
                groups.forEach(group => {
                    (group.files || []).forEach(file => {
                        const existing = filesMap.get(file.name);
                        if (existing) {
                            existing.diffElements.push(...(file.diffElements || []));
                        } else {
                            filesMap.set(file.name, { name: file.name, diffElements: [...(file.diffElements || [])] });
                        }
                    });
                    tools = tools.concat(group.tools || []);
                    thoughts = thoughts.concat(group.thoughts || []);
                    questions = questions.concat(group.questions || []);
                });
            }

            const aggregateGroup = {
                id: 'day-' + (sessoes[0] ? sessoes[0].filename : ''),
                timestamp: sessoes[0] ? (sessoes[0].datetime || '') : '',
                title: 'Dia',
                tools,
                thoughts,
                questions,
            ...
            openFilesPanel(aggregateGroup);
        } catch (e) {
            console.error('Erro ao atualizar arquivos do dia:', e);
        }
    }
